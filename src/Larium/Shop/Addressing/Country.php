...
final class Country
{
    ...
    public function __construct(
        ...
        $args = $this->normalizeInput(func_get_args());
    ...
    private function find($args)
    {
        ...
        if (empty($item)) {
            throw new \InvalidArgumentException(
                sprintf('Country with data `%s` does not exist.', implode(', ', $args))
            );
        }
        ...
        $this->numeric = $item['numeric'];
    }

    private function normalizeInput($args)
    {
        $params = array(
            'name',
            'alpha2',
        ...
        $inputArgs = array_replace([null, null, null, null], $args);

        return array_filter(
            array_combine($params, $inputArgs)
        );
    }
